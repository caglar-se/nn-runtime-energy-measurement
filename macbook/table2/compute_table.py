...
compute_table.py
================
Reads the CSVs produced by run_measurements.sh and prints Table 2:

  NNVC decoder (LOP7 off / on) + isolated LOP7 single-patch CPU measurements
  (macOS / Apple Silicon), with derived overhead, N× single-patch, and
  decoder/N× ratio. No GPU section — Apple Silicon has no discrete GPU.

Usage
-----
  python macbook/table2/compute_table.py \\
      --decoder_csv  macbook/table2/results/decoder.csv \\
      --cpu_csvs     macbook/table2/results/lop7_cpu_run*.csv \\
      --patches_per_frame 24 \\
      --output_md    macbook/table2/results/table2.md
"""
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="Aggregate measurement CSVs and print Table 2 (macOS, CPU only).",
    ...
    p.add_argument("--cpu_csvs", nargs="+", required=True,
                   help="CSV(s) written by measure_cpu_macos.py (one per run)")
    ...
    n_cpu = len(cpu_df)
    stats_note = f"decoder {n_dec} run(s), CPU {n_cpu} run(s)"
    single_run_warning = (
        "Note: std is 0 for single-run measurements. Use N_RUNS ≥ 5 for reliable ±."
        if (n_dec < 2 or n_cpu < 2) else ""
    )
    ...
    print()
    print(f"  Table 2 — MacBook Apple Silicon (CPU only)")
