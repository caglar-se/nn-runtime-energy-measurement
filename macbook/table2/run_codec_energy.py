...
For each stage (encoder, decoder):
  ...
  3. Measurement: run subprocess in a loop for --measure_duration seconds
                  bracket the whole window with RAPL / NVML / powermetrics counters
...
Usage (macOS / Apple Silicon, no discrete GPU)
-----------------------------------------------
  cd /path/to/nn-runtime-energy-measurement
  python3 macbook/table2/run_codec_energy.py \\
      --vtm_dir  /path/to/VVCSoftware_VTM \\
      --input_yuv /path/to/VVCSoftware_VTM/CTC_Image/yuv_output_ctc/kodim19_512x768_8bit_420.yuv \\
      --base_cfg cfg/encoder_intra_nnvc.cfg \\
      --seq_cfg  cfg/CTC_JPEGAI/kodim19_512x768_8bit_420.cfg \\
      --frames 1 --qp 42 --runs 10 \\
      --gpu_index -1 --energy_backend powermetrics \\
      --output_csv macbook/table2/results/decoder.csv
